...
void print_tryhelp(const char command[]) {
    fprintf(stderr, "MPI Try %s -help for more info\n", command);
}

void print_help(const char command[]) {
    fprintf(stderr, "%s: MPI Simulate a 2d wave equation over a velocity field.\n\n", command);
    ...
    fprintf(stderr, "%-18s %s\n", "-dt     <double>", "DT. Default 0.000133333s");
    ...
    fprintf(stderr, "%-18s %s\n", "-vel <double>",     "Velocity model constant speed");
    fprintf(stderr, "%-18s %s\n", "-model <modelfile>",     "File name of velocity model");
    fprintf(stderr, "\nSource Parameters:\n\n");
    ...
    fprintf(stderr, "%-18s %s\n", "-source <sourcefile>",     "File name of wavelet source");
}
