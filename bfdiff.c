/*
 * bfdiff   Performs a binary difference in float files
 */
...
int main(int argc, char *argv[])
{
    ...
    while((bytes_read_buf = fread(buf, sizeof(float), BUFFER, fp)) &&
          (bytes_read_stdin =fread(bufin, sizeof(float), BUFFER, stdin)))
    {
